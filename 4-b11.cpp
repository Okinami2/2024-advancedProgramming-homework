...
int main()
{
	...
	cout << setw((end_ch - 'A') * 2 + 1) << setfill('=') << '=' << endl; /* 按字母塔最大宽度输出=(不允许用循环) */
	print_tower(end_ch, true); //正序打印 A~结束字符 
	cout << endl;
	...
	cout << setw((end_ch - 'A') * 2 + 1) << setfill('=') << '=' << endl; /* 按字母塔最大宽度输出=(不允许用循环) */
	print_tower(end_ch, false); //逆序打印 A~结束字符 
	cout << endl;
	...
	cout << setw((end_ch - 'A') * 2 + 1) << setfill('=') << '=' << endl;/* 按字母塔最大宽度输出= */
	print_tower(end_ch, true);   //打印 A~结束字符的正三角 
	print_tower(end_ch - 1, false);   //打印 A~结束字符-1的倒三角 
	cout << endl;
